...
`cooperative-only` waits are Python methods such as `join()`, `Future.result()`,
`Event.wait()`, queue operations, and pool result methods that can advance only
while Pyodide can run the browser event loop through JSPI `run_sync`. They do
not preempt running Python bytecode. This module converts missing runtime
support into `UnsupportedWasmConcurrencyError` and lets user callback
