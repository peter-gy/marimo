...
def test_blob_asset_resolves_to_asset_codec() -> None:
    asset = BlobAsset(
        data=b"<svg></svg>",
    ...
    assert maybe_update_lazy_stub(asset) == "asset"
...
def test_blob_asset_codec_round_trip() -> None:
    ...
    encoded = BLOB_SERIALIZERS["asset"](asset)

    assert BLOB_DESERIALIZERS[".asset"](encoded, None) == asset


def test_lazy_loader_round_trips_blob_asset() -> None:
    store = MockStore()
    loader = LazyLoader("asset-test", store=store)
    asset = BlobAsset(
        data=b"\x89PNG\r\n\x1a\n",
    ...
    assert (
        Path("asset-test") / "asset_hash" / "preview.asset"
    ).as_posix() in store._cache
    assert (
        Path("asset-test") / "asset_hash" / "return.asset"
    ).as_posix() in store._cache
